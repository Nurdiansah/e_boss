            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        <h4 class="card-title">Daftar Port</h4>
                        ...
                        <div class="modal fade" id="modalAdd" tabindex="-1" role="dialog" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <form action="{{route('port.store')}}" name="form" method="POST">
                                        @csrf
                    ...
                    <div class="table-responsive">
                        <table id="basic-datatables" class="display table table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td> <a href="{{route('port.show', $port->id)}}"> {{$port->name}} </a></td>
                                    <td>{{$port->created_at}}</td>
                                    <td>{{$port->updated_at}}</td>
                                    <td>
                                        <!-- route('port.edit', $port->id -->
                                        <a href="">
                                            <a href="{{route('port.edit', $port->id)}}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                                            <button class="btn btn-danger" data-toggle="modal" data-target="#modalDelete&id={{ $port->id }}"><i class="fa fa-trash"></i></button>
                                        </a>
                                    </td>
                                </tr>

                                <!-- Modal Delete -->
                                <div class="modal fade" id="modalDelete&id={{ $port->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{route('port.delete', $port->id)}}" name="form" method="POST">
                                                @method('DELETE')
                                                @csrf
                                                <input type="hidden" name="id" value="">
                                                <div class="perhitungan">
                                                    <div class="modal-body">
                                                        <h4 class="text-center">Apakah anda yakin ingin menghapus port {{$port->name}} ?</h4>
                                                    </div>
                                                    <div class="modal-footer ">
