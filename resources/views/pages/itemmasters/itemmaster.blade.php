            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        <h4 class="card-title">Daftar Item Master</h4>
                                    ...
                                    <div class="modal-header no-bd bg-primary">
                                        <h5 class="modal-title">
                                            <span class="fw-mediumbold">
                                                Add Item Master</span>
                                        </h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <form action="{{route('itemmaster.store')}}" name="form" method="POST">
                                        @csrf
                    ...
                    <div class="table-responsive">
                        <table id="basic-datatables" class="display table table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td> <a href="{{route('itemmaster.show', $itemmaster->id)}}"> {{$itemmaster->name}} </a></td>
                                    <td>{{$itemmaster->created_at}}</td>
                                    <td>{{$itemmaster->updated_at}}</td>
                                    <td>
                                        <!-- route('itemmaster.edit', $itemmaster->id -->
                                        <a href="">
                                            <a href="{{route('itemmaster.edit', $itemmaster->id)}}"><button class="btn btn-success"><i class="fa fa-edit"></i></button></a>
                                            <button class="btn btn-danger" data-toggle="modal" data-target="#modalDelete&id={{ $itemmaster->id }}"><i class="fa fa-trash"></i></button>
                                        </a>
                                    </td>
                                </tr>

                                <!-- Modal Delete -->
                                <div class="modal fade" id="modalDelete&id={{ $itemmaster->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header no-bd bg-danger">
                                                <h5 class="modal-title">
                                                    <span class="fw-mediumbold">
                                                        Delete Item Master</span>
                                                </h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{route('itemmaster.delete', $itemmaster->id)}}" name="form" method="POST">
                                                @method('DELETE')
                                                @csrf
                                                <input type="hidden" name="id" value="">
                                                <div class="perhitungan">
                                                    <div class="modal-body">
                                                        <h4 class="text-center">Apakah anda yakin ingin menghapus equipment {{$itemmaster->name}} ?</h4>
                                                    </div>
                                                    <div class="modal-footer ">
